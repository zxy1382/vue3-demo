...
export function drawExtent() {
  ...
  map.graphicLayer.startDraw({
    ...
    success: function (graphic) {
      console.log(graphic);
      //获取多边形的坐标范围
      //   const polygon = mars3d.PolyUtil.formatPolygon(graphic._positions_draw);

      // 获取矩形的坐标范围
      //   const rectangle = mars3d.PolyUtil.formatRectangle(
      //     graphic._rectangle_draw
      //   );
      eventTarget.fire("drawExtent", {
        extent: JSON.stringify(graphic._positions_draw),
      }); // 抛出事件，可以组件中去监听事件
    },
  });
}
